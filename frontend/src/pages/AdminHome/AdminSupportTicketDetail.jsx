...
export default function AdminSupportTicketDetail() {
  ...
  const setStatus = async (status) => {
    ...
    setError('');
    try {
      const data = await patchSupportTicketStatus(id, status);
      setTicket(data.ticket || null);
    } catch (err) {
  ...
  const storeName = ticket?.storeId?.name || '—';
              ...
              <div className="manager-panel-card manager-products-card" style={{ marginBottom: 16 }}>
                <div style={{ padding: '1rem' }}>
                  <p style={{ margin: '0 0 8px', color: '#64748b', fontSize: 14 }}>
                    ...
                    {' · '}
                    Người gửi: <strong>{sender}</strong>
                  </p>
              ...
              <div className="manager-panel-card manager-products-card">
                ...
                <div style={{ padding: '0 1rem 1rem' }}>
                  {(ticket.replies || []).length === 0 ? (
                    ...
                    <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                      {ticket.replies.map((r) => {
                        const name =
                          r.userId?.fullName || r.userId?.email || (r.role === 'admin' ? 'Admin' : 'Quản lý');
                        return (
                          <li
                            key={r._id}
                            style={{
                  ...
                  {ticket.status !== 'closed' ? (
                    ...
                    <p style={{ marginTop: 16, color: '#64748b' }}>Phiếu đã đóng. Mở lại để tiếp tục trả lời.</p>
                  )}
                </div>
              </div>

              <div style={{ marginTop: 16 }}>
                <Link to="/admin/support" className="manager-btn-outline" style={{ textDecoration: 'none' }}>
                  Về danh sách
                </Link>
              </div>
            </>
